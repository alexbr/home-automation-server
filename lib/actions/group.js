...
function addToGroup(player, values) {
  ...
  if (!joiningPlayer) {
    loggerJs.warn(`Room ${joiningRoomName} not found - can't group with ${player.roomName}`);
...
function joinPlayer(player, values) {
  ...
  if (!receivingPlayer) {
    loggerJs.warn(`Room ${receivingRoomName} not found - can't make ${player.roomName} join it`);
