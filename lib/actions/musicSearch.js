...
function getAccountId(player, service) {
   ...
   if (service != 'library') {
      console.warn('making request to ', player.baseUrl + '/status/accounts');
      return request({
      ...
      if (serviceDef.empty(type, resList)) {
         ...
         if (type === 'station') {
            ...
            return player.coordinator.setAVTransport(UaM.uri, UaM.metadata).then(() => {
               console.warn('playing this shiIIIIIIIIIIIIIIIIIIIt!');
            ...
            if (tracks.count === 0) {
               ...
               if (tracks.isArtist) {  // Play numerous songs by the specified artist
                  ...
                  logger.info('first track:', tracks.queueTracks[0]);
                  var empty = false;
                  var nextTrackNo = 0;

                  return player.coordinator.getQueue(0, 1).then(queue => {
                     empty = queue.length === 0;
                     logger.info('current track number: ', player.coordinator.state.trackNo);
