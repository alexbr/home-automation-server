...
const musicServices = new Set(['apple', 'spotify', 'deezer', 'elite', 'library']);
const serviceNames = {
...
function getAccountId(player, service) {
	...
	if (service !== 'library') {
		...
		return request(request_).then(res => {
...
function doSearch(service, type, term) {
	...
	return authenticate().then(() => request(options));
}
...
function search(player, values) {
	...
	return getAccountId(player, service).then(() => doSearch(service, type, term)).then(resList => {
		const serviceDef = getService(service);
