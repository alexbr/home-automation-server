...
function saveAll(system) {
  ...
  loggerJs.trace('backup presets', backupPresets);
  ...
  return system.applyPreset(preset)
    ...
    .then(() => {
      ...
      return new Promise((resolve) => {
        const transportChange = (state) => {
          loggerJs.debug(`Player changed to state ${state.playbackState}`);
        ...
        loggerJs.debug(`Setting restore timer for ${restoreTimeout} ms`);
    ...
    .then(() => {
      return backupPresets.reduce((promise, preset) => {
        loggerJs.trace('Restoring preset', preset);
        return promise.then(() => system.applyPreset(preset));
      }, Promise.resolve());
    })
    .catch((err) => {
      loggerJs.error(err.stack);
      throw err;
    });
