...
AlexaHandler.prototype.getIntentHandlers = function() {
   ...
   return {
      ...
      FavoriteIntent: function() {
         ...
         console.warn(`found preset ${preset}`);
         if (!preset) {
            return this.emit(':delegate');
         }
      ...
      SetSleepIntent: function() {
         ...
         self.loadRoomAndService(deviceId, intent.slots.Room.value).then(res => {
            ...
            return self.sonosAction('sleep', room, [ durationSecs ]).then(() => {
               self.okOrSpeakAndFinish(response, `Ok. Sleeping in ${msg}`);
            });
         }).catch(error => {
            self.error(response, error);
         });
...
AlexaHandler.prototype.okOrSpeakAndFinish = function(response, msg) {
   if (shortResponse) {
      this.ok(response);
   } else {
...
console.warn(handler.event.context);
   return {
