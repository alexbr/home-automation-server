...
AlexaHandler.prototype.getIntentHandlers = function() {
   ...
   return {
      CanFulfillIntentRequest: function() {
         logger.info('CanFulfillIntentRequest received');
         const { intent, response } = self.getStuff(this);
      ...
      AlbumIntent: function() {
         ...
         const { intent, response, deviceId } = self.getStuff(this);
      ...
      TrackIntent: function() {
         logger.info('TrackIntent received');
      ...
      SearchIntent: function() {
         logger.info('SearchIntent received');
         const { intent, response, deviceId } = self.getStuff(this);
         ...
         return self.loadRoomAndService(deviceId, intent.slots.Room.value).then(res => {
            ...
            if (selectedTrack) {
               ...
               const song = _.get(intent.slots, 'Name.value');
               if (!song) {
                  return this.emit(':delegate');
               }
               ...
               return self.sonosAction('search', room, values).then(res => {
                  ...
                  if (!tracks || tracks.length === 0) {
                     ...
                     if (tracks.length === 1) {
                        ...
                        if (!foundTrack) {
                           const question = `Choose a track: ${msg}`;
      ...
      PauseAllIntent: function() {
         const { response } = self.getStuff(this);
      ...
      VolumeDownIntent: function() {
         ...
         const stuff = self.getStuff(this);
         ...
         self.loadRoomAndService(deviceId, intent.slots.Room.value).then(res => {
            ...
            return self.sonosAction('state', room).then(res => {
               ...
               if (currentTrack.title.startsWith('x-sonosapi')) {
                  ...
                  responseText = stateResponses[Math.floor(Math.random() * stateResponses.length)].replace(/&/g, 'and');
               }
      ...
      AmpVolumeUpIntent: function() {
         logger.info('AmpVolumeUpIntent received');
         const stuff = self.getStuff(this);
         return self.sendAmpCommand(stuff, 'volup', {amount: 5}).then(() => {
            self.ok(stuff.response);
         }).catch(() => {
            this.speakAndFinish(stuff.response, `Sorry that didn't work.`);
         });
      },

      AmpVolumeDownIntent: function() {
         logger.info('AmpVolumeDownIntent received');
         const stuff = self.getStuff(this);
         return self.sendAmpCommand(stuff, 'voldown', {amount: 5}).then(() => {
            self.ok(stuff.response);
         }).catch(() => {
            this.speakAndFinish(stuff.response, `Sorry that didn't work.`);
         });
      ...
      AmpMuteIntent: function() {
         logger.info('AmpMuteIntent received');
         const stuff = self.getStuff(this);
         return self.sendAmpCommand(stuff, 'mute').then(() => {
            self.ok(stuff.response);
         }).catch(() => {
            this.speakAndFinish(stuff.response, `Sorry that didn't work.`);
         });
      ...
      AmpTunerIntent: function() {
         logger.info('AmpTunerIntent received');
         const stuff = self.getStuff(this);
         return self.sendAmpCommand(stuff, 'tuner').then(() => {
            self.ok(stuff.response);
         }).catch(() => {
            this.speakAndFinish(stuff.response, `Sorry that didn't work.`);
         });
      ...
      AmpPhonoIntent: function() {
         logger.info('AmpPhonoIntent received');
         const stuff = self.getStuff(this);
         return self.sendAmpCommand(stuff, 'phono').then(() => {
            self.ok(stuff.response);
         }).catch(() => {
            this.speakAndFinish(stuff.response, `Sorry that didn't work.`);
         });
...
AlexaHandler.prototype.sendAmpCommand = function(stuff, cmd, options) {
   ...
   return this.loadRoomAndService(deviceId, intent.slots.Room.value).then(response => {
      const { room } = response;
