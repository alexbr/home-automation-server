...
AlexaHandler.prototype.getIntentHandlers = function() {
   ...
   return {
      ...
      AmpOnIntent: function() {
         logger.info('AmpOnIntent received');
         const stuff = self.getStuff(this);
         return self.sendAmpCommand(stuff, 'pwron').then(() => {
      ...
      AmpOffIntent: function() {
         logger.info('AmpOffIntent received');
         const stuff = self.getStuff(this);
         return self.sendAmpCommand(stuff, 'pwroff').then(() => {
            self.ok(stuff.response);
         }).catch(() => {
            this.speakAndFinish(stuff.response, `Sorry that didn't work.`);
         });
      ...
      AmpPhonoIntent: function() {
         ...
         return self.sendAmpCommand(stuff, 'phono').then(() => {
            return self.loadRoomAndService(deviceId, intent.slots.Room.value);
         }).then(res => {
            const { room } = res;
